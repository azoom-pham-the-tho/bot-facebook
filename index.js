...
const handlerDefault = async (command) => {
  ...
  if (maxScore >= 0.6) {
    ...
    if (dataTrainning[index].fileName) {
      await command.message.sendAttachment({
        title: dataTrainning[index].answer,
        url_or_path: fomatFilePath(dataTrainning[index].fileName)
      })
      if(dataTrainning[index].link){
        command.message.send({
          body: dataTrainning[index].link,
          url: dataTrainning[index].link
        })
      }
    } else {
...
const getUserInfo = async (userId) => {
  const user = usersHistory.get(userId)
  if(user) return user
